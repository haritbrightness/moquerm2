...
class Sitemap extends \Mageplaza\Sitemap\Block\Sitemap
{
    ...
    public function __construct(
        ...
        parent::__construct($context,$categoryHelper,$collection,$categoryCollection,$categoryRepository,$helper,$stockFilter,$productVisibility,$productCollection,$pageCollection);
    }
     ...
     * @return string
     */
    public function renderLinkElement($link, $title,$level)
    {
        return '<li><a class = "sitemap-level-'.$level.'" href="' . $link . '">' . __($title) . '</a></li>';
    }
        ...
        if ($config) {
            ...
            if ($collection) {
                ...
                foreach ($collection as $key => $item) {
                    switch ($section) {
                        case 'category':
                            $html .= $this->renderLinkElement($this->getCategoryUrl($item->getId()), $item->getName(),$item->getLevel());
                            break;
                        case 'page':
                            if (in_array($item->getIdentifier(), $this->getExcludedPages())) {
                                continue 2;
                            }
                            $html .= $this->renderLinkElement($this->getUrl($item->getIdentifier()), $item->getTitle(),$item->getLevel());
                            break;
                        case 'product':
                            $html .= $this->renderLinkElement($this->getUrl($item->getProductUrl()), $item->getName(),$item->getLevel());
                            break;
                        case 'link':
                            $html .= $this->renderLinkElement($key, $item,0);
                            break;
                    }
                }
                $html .= '</ul>';
