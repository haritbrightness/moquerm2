 ...
 * Class Invoice
 */
class Invoice extends \Aitoc\OrdersExportImport\Model\Processor\EntityType\AbstractEntity
{
    /**
     ...
     */
    protected $model = \Magento\Sales\Model\Order\Invoice::class;
    ...
    public function __construct(
        ...
        \Magento\Sales\Model\Order\Invoice\ItemFactory $itemFactory,
        \Magento\Sales\Model\Order\Invoice\CommentFactory $commentFactory
    ) {
    ...
    protected function prepareExtras()
    {
        ...
        $extra = [
            self::INVOICE_ITEM          => $this->getEntity()->getItemsCollection(),
            self::INVOICE_COMMENT       => $this->getEntity()->getCommentsCollection(),
        ];
    ...
    protected function validateEntity($data)
    {
        ...
        if (empty($data['item'])) {
            throw new \Exception(__('Invoice items are missing'));
        }
    }
    ...
    private function validateIncrement($entityData)
    {
        if (empty($entityData['increment_id'])) {
            throw new \Exception(
                __("Invoice have not Increment Id")
            );
            ...
            if ($object->getId()) {
                throw new \Exception(__('Invoice #%1 already exist.', $object->getIncrementId()));
            }
        }
    }
    ...
    private function addItems($items)
    {
        ...
        foreach ($items as $item) {
            ...
            // link invoice item with order item by name
