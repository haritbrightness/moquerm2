...
class Add extends \Magento\Checkout\Controller\Cart\Add
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if ($product->getTypeId() == Configurable::TYPE_CODE) {
                $simple = $this->configurable->getProductByAttributes(
                    $params['super_attribute'],
                    $product
                );
                $this->setQuoteProduct($simple);
                if ((bool)$this->helper->getModuleConfig('confirm_display/configurable_image')) {
                    $this->_coreRegistry->register('amasty_cart_conf_product', $simple);
                } else {
    ...
    protected function isShowOptionResponse($product, $params)
    {
        ...
        switch ($product->getTypeId()) {
            ...
            case 'grouped':
                ...
                break;
            case 'amgiftcard':
                if (!array_key_exists('am_giftcard_sender_email', $params)) {
                    $showOptionsResponse = true;
                }
                break;
            case 'bundle':
    ...
    private function getQtyBlockHtml()
    {
        ...
        if ($this->helper->isChangeQty()) {
            $quoteItem = $this->getItemByProduct($this->getQuoteProduct(), $this->getCartModel()->getQuote());
            if ($quoteItem) {
