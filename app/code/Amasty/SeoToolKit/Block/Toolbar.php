...
class Toolbar extends Template
{
    ...
    {
        $html = $this->getPageHtml();
        $text = __('There are no canonical tag is set for this page');
        ...
        if (preg_match_all('/<title>(.*?)<\/title>/s', $html, $result)) {
            if (isset($result[0]) && isset($result[1][0])) {
                ...
                if ($count > self::OPTIMAL_TITLE_LENGTH) {
                    $text = __('%1 characters — Google typically displays the first 50–60 characters of a title tag. If you keep your titles under 60 characters, our research suggests that you can expect about 90% of your titles to display properly.', $count);
                } else if ($count) {
    ...
    public function getMetaDescCount()
    {
        ...
        if (preg_match_all('/<meta\s+name="description"\s+content="(.*?)".*>/s', $html, $result)) {
            if (isset($result[0]) && isset($result[1][0])) {
                ...
                $count = strlen($this->metaDescription);
                $text = __('%1 characters — optimal.', $count);
                $status = self::NORMAL_CLASS;
            }
        }
    ...
    public function getKeywordCount()
    {
        ...
        if (preg_match_all('/<meta\s+name="keywords"\s+content="(.*?)".*>/s', $html, $result)) {
            if (isset($result[0]) && isset($result[1][0])) {
                ...
                $count = strlen($this->keywords);
                if ($count > self::OPTIMAL_KEY_LENGTH) {
                    $text = __('%1 characters —  When writing meta descriptions, we tend to keep it between 150-160 characters because Google usually truncate snippets that are longer than 160 characters.', $count);
                } else if ($count) {
                    $text = __('%1 characters — optimal.', $count);
                }
                $status = self::NORMAL_CLASS;
        ...
        if (preg_match_all('/<a\s+href=["\']?([^"\'>]+)["\']?.*?>/s', $html, $result)) {
            if (isset($result[0]) && isset($result[1])) {
                ...
                $noFollow = array_filter($links, function($value) {
                    return strpos($value, 'rel="nofollow"') !== false || strpos($value, "rel='nofollow'") !== false;
                });
                ...
                $external = array_filter($result[1], function($url) {
                    ...
                    return isset($urlData['host']) && $urlData['host'] !== $this->request->getHttpHost();
                });
